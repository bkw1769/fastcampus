...
const generateGeminiRoadmap = async (skills, goal) => {
  ...
  const prompt = `
    역할: 당신은 IT 개발자 전문 커리어 코치입니다.
    사용자 현재 스킬: ${skills.join(', ')}
    목표 직무: ${goal}
    
    목표 직무로 전환하기 위해 가장 효율적이고 실무적인 학습 로드맵을 제안해주세요.
    단계를 인위적으로 3단계로 제한하지 말고, 필요하다면 4~6단계까지 구체적으로 나눠주세요.
    각 단계는 실무 프로젝트나 구체적인 기술 스택 학습을 포함해야 합니다.
    
    반드시 아래 JSON 형식으로만 응답해주세요 (Markdown 코드 블록 없이 순수 JSON만 반환).
    모든 내용은 한국어로 작성해주세요.

    {
      "steps": [
        {
          "short_title": "단계명 (핵심 키워드, 공백포함 12자 이내)", 
          "description": "구체적으로 무엇을 학습하거나 어떤 프로젝트를 해야 하는지 1~2문장으로 설명"
        },
        ... (필요한 만큼 단계 추가)
      ],
      "message": "사용자의 도전을 응원하는 따뜻하고 동기부여되는 메시지 (50자 이내)"
    }
  `;
